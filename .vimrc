...
set list                                              "开启显示不可见字符
set listchars=tab:>-,trail:-                          "设置tab和空格显示
highlight WhitespaceEOL ctermbg=red guibg=red 
match WhitespaceEOL /\s\+$/                           "不显示换行符
...
" 启用每行超过100列的字符提示（字体变灰并加下划线），不启用就注释掉
...
"set statusline=%F%m%r%h%w\ [%{&fenc!=''?&fenc:&enc}:%{&ff}]\ [TYPE=%Y]\ [POS=%l,%v][%p%%]\ %{strftime(\"%d/%m/%y\ -\ %H:%M\")} "状态栏
set cmdheight=2                                       "设置命令行的高度为2，默认为1
set cursorline                                        "突出显示当前行
set guifont=Consolas::h14:cANSI                       "设置字体:字号（字体名称空格用下划线代替）
set nowrap                                            "设置不自动换行
...
" 常规模式下输入 <F4> 调用插件，如果有打开 TagList 窗口则先将其关闭
